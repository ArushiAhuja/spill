...
import { getPromptMetadata, PROMPT_KEYS } from '../../../../../server/prompts.js';
...
export async function POST(request) {
  try {
    ...
    const [{ rows: categories }, prompt] = await Promise.all([query('SELECT id,name,description,severity FROM categories WHERE org_id=$1',[org_id]), getPromptMetadata(org_id,prompt_key)]);
    const system = prompt_override?.trim() || prompt.content;
